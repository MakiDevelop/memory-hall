...
- **`MH_API_TOKEN` 與 `MH_ADMIN_TOKEN` 必須是不同值**：操作者責任，code 不強制驗證（驗證會反 ADR 0008 輕量原則，加複雜度大於收益）
...
- `tests/test_auth.py`：6 個新 case（admin_token unset fallback / admin_token set correct / api_token rejected on admin / missing header / admin_token 不能用在非 admin path / health 仍 public）
- `.env.example`：加 `MH_ADMIN_TOKEN=` 範例段落
- `docs/api.md`：加「Admin gate (two-tier bearer)」段落

Total: ~120 行 across 6 files。`pytest`：14 passed (auth)，full suite 待跑驗證無 regression。
